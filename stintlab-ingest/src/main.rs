...
async fn main() -> Result<()> {
    ...
    let client =
        openf1::OpenF1Client::new().context("failed to create OpenF1 client")?;
    ...
    for (idx, session) in sessions.iter().enumerate() {
        ...
        let mut race = match normalize::session_to_race(session, round) {
            Some(r) => r,
            None => {
                warn!(session_key = session.session_key, "skipping: incomplete session data");
                continue;
            }
        };
        ...
        let lap_count = storage::insert_laps(&conn, &laps)
            .context("failed to insert laps")?;
        let stint_count = storage::insert_stints(&conn, &stints)
            .context("failed to insert stints")?;

        info!(lap_count, stint_count, "inserted data");

        // Fit degradation models for each compound at this circuit
        let compounds_seen: HashSet<Compound> =
            laps.iter().map(|l| l.compound).collect();
